...
#     connection = pymysql.connect(host='localhost', user='root', password='', database='Guango')
...
@app.before_request
def before_request():
    """Ensure the connection is open before each request."""
    get_db_connection()
...
def register():
    if request.method == 'POST':
        ...
        # Insert the new user
        sql = "INSERT INTO users (username, phone, email, password, role) VALUES (%s, %s, %s, %s, %s)"
        cursor.execute(sql, (username, phone, email, password, role))
        connection.commit()
        cursor.close()
...
def admin():
    ...
    if request.method == 'POST':
        ...
        sql = "INSERT INTO products (category, name, price, stock) VALUES (%s, %s, %s, %s)"
        cursor.execute(sql, (category, name, price, stock))
        connection.commit()
        cursor.close()
...
def cart():
    ...
    for product_id, quantity in session['cart'].items():
        ...
        if product:
            ...
            product.append(quantity)  # Add quantity to product details
            cart_items.append(product)
            total_price += product[2] * quantity  # Assuming price is the 3rd column
...
def checkout():
    ...
    # Process the payment (Mpesa, etc.)
    # Here, you can add your payment processing logic
    
    # Clear the cart after payment
    session.pop('cart', None)
    flash('Payment successful! Thank you for shopping with us.', 'success')
    return redirect('/index')
